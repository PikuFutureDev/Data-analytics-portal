...
#title
st.title(':red[Data] :blue[Analytics] :violet[Portal]')
st.subheader(':gray[Explore Data with ease.]',divider='orange')
...
if(file!=None):
    ...
    st.subheader(':green[Basic Information Of The Dataset]',divider='rainbow')
    tab1,tab2,tab3,tab4 =st.tabs(['Summary','Top and Bottom Rows','Data Types','Columns'])
    with tab1:
        st.write(f'There are{data.shape[0]}rows in dataset and {data.shape[1]}colums in the dataset')
    ...
    st.subheader(':green[Column Values To Count]',divider='rainbow')
    ...
    with st.expander('Group By your columns'):
        ...
        if(groupby_cols):
            ...
            st.dataframe(result)
            st.subheader(':orange[Data Visualization]',divider='gray')
